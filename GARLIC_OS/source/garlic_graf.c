...
void _gg_procesarFormato(char *formato, unsigned int val1, unsigned int val2, char *resultado)
{
	...
	while(formato[i] != '\0')
	{
		if(formato[i] == '%')	//identifiquem un format
		{
			...
			/*
			else if(formato[i+1] == 'l' || formato[i+1] == 'L')	//si es tracta d'un long long
			{
				char num[20];
				int index = 0;
				int error;
				...
				//decidim quin val fem servir
				if(!valActual) error = _gs_long2str_dec(num, sizeof(num), val1);
				else error = _gs_long2str_dec(num, sizeof(num), val2);
				...
				if(error == 0)	//en cas de que la funcio s'hagi pogut executar be...
				{
					...
					else //volem separacio per punts
					{
						index = sizeof(num) - 1; //no ens fa falta fer res amb el '\0'
						while(num[index] != ' ')
						{
							if(index % 3 == 0)
								resultado[res_i] = '.';	//punt cada 3 dígits
							else
							{
								resultado[res_i] = num[index];	//guardem el numero dec a l'string final
								index++;
...
void _gg_escribir(char *formato, unsigned int val1, unsigned int val2, int ventana)
{
	...
	cPendents = VCOLS - (_gd_wbfs[ventana].pControl & 0x0000ffff);	//obtenim el numero d'elements al vector
	...
	while(text[i] != '\0')
	{
		...
		if(final)
		{
			_gd_wbfs[ventana].pControl &= 0x0000ffff;
			_gd_wbfs[ventana].pControl |= (fila << 16);
			_gd_wbfs[ventana].pControl &= 0xffff0000;
		}
		else
		{
			_gd_wbfs[ventana].pControl &= 0xffff0000;
