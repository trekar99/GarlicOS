/*------------------------------------------------------------------------------
	...
	Programador P: albert.canadilla@estudiants.urv.cat
	Programador M: gerard.pascual@.urv.cat
	Programador G: jaume.tello@estudiants.urv.cat
	Programador T: german.puerto@estudiants.urv.cat
...
/* _gm_pintarFranjas: rutina para pintar las l?neas verticales correspondientes
	...
	Parametros:
		zocalos		->	vector con numeros de zocalo de los procesos que estan
...
*/
extern void _gm_pintarFranjas(char zocalos[], unsigned short index_ini,
...
//------------------------------------------------------------------------------
//	Rutinas de soporte a la gestión de teclado (garlic_tecl.c)
//------------------------------------------------------------------------------

/* _gt_initKB:  función para inicializar toda la información necesaria para la 
			   gestión del teclado, como las variables globales, los gráficos, 
			   instalar la RSI de teclado, etc. */
extern void _gt_initKB();

/* _gt_showKB:  función para mostrar la interfaz de teclado.*/
extern void _gt_showKB();

/* _gt_showKB:  función para ocultar la interfaz de teclado.*/
extern void _gt_hideKB();

/* _gt_setcapsKB:  función para visualizar el teclado táctil.*/
extern void _gt_setcapsKB();

/* _gt_rsi_KEYS_XY:  rutina de servicio de interrupción de captación de XY.*/
extern void _gt_rsi_KEYS_XY();

/* _gt_rsi_IPC_FIFO:  rutina de servicio de interrupción de captación táctil.*/
extern void _gt_rsi_IPC_FIFO();

/* _gt_make_q12:  rutina de para pasar parte entera y parte decimal a Q12.*/
extern int _gt_make_q12(char signo, int entero, int decimal);

/* _gt_bgcolors_reset:  rutina para resetear colores input teclado.*/
extern void _gt_bgcolors_reset();

/* _gt_bgcolors_reset:  rutina para resetear colores input teclado.*/
extern void _gt_bgcolors_put(int pos);

/* _gt_kb_check_type:  rutina para checkear las pulsaciones segun tipo teclado.*/
extern char _gt_kb_check_type(int type, int pos, char ascii);

/* _gt_highlightKB:  rutina para checkear las pulsaciones segun tipo teclado.*/
extern void _gt_highlightKB(int mes, int ascii);

//------------------------------------------------------------------------------
//	Variables de soporte a la gestión de teclado (garlic_tecl.c)
//------------------------------------------------------------------------------

/* Variables para ubicar string de teclado */
#define INPUT_LINE 3
#define INPUT_COLS 32
#define INPUT_SITE INPUT_LINE * INPUT_COLS

/* Variables de control límites string */
#define LIM_LEFT 1
#define LIM_RIGHT 30

/* Variables de colores de paleta */
#define COLOR_WHITE 0
#define COLOR_BLUE 128
#define COLOR_PINK 256
#define COLOR_SALMON 384

/* Variables de letras tiles */
#define ASCII_TILE 95

/* _gt_mapbgKB:  puntero al mapa de baldosas de la interfaz de teclado.*/
extern u16* _gt_mapbgKB;

/* _gt_mapbgKB:  puntero al mapa de baldosas del cursor de teclado.*/
extern u16* _gt_mapbgCursorKB;

/* _gt_mapbgKB:  puntero al mapa de fondo de colores del teclado.*/
extern u16* _gt_mapbgColors;  

/* _gt_mapbgKB: indice al mapa de tablas de procesos.*/
extern int _gt_bgtable;  

/* _gt_activeKB: variable que indica si el teclado está activado.*/
extern int _gt_activeKB;

/* _gt_capsKB: variable que indica si las teclas especiales están activadas.*/
extern int _gt_capsKB;

/* _gt_xybuttons: variable que indica si las teclas XY están activadas.*/
extern int _gt_xybuttons;

/* _gt_FIFO_RECV_status: variable que indica si hay puls táctil recibido.*/
extern int _gt_FIFO_RECV_status;
